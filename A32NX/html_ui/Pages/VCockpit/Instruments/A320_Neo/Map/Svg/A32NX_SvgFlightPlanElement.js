    ...
    makePathFromWaypoints(waypoints, map) {
        ...
        if (points.length > 2) {
            ...
            for (let i = 1; i < points.length - 1; i++) {
                ...
                console.log(p.refWP.ident + ` => from=${hdgPrev} to=${hdgNext} deltaPc=${deltaPc} xKR=${xKr}`);
        ...
        for (const circle of circles) {
            ...
            //pathString += ` M ${fastToFixed(cx - r, 2)}, ${fastToFixed(cy, 2)} a ${fastToFixed(r, 2)},${fastToFixed(r, 0)} 0 1,0 ${fastToFixed(2 * r, 2)},0 a ${fastToFixed(r, 2)},${fastToFixed(r, 2)} 0 1,0 ${fastToFixed(-2 * r, 2)},0`;
        }
